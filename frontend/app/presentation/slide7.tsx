...
type AnalysisComparison = {
  example: string;
  label: ReportImage;
  inference: ReportImage;
};

const round3Models: ModelReport[] = [
  {
    id: 'yolov8s-r3', round: 'ROUND 03', name: 'YOLOv8s', epochs: '100', map50: '94.36%', map95: '81.52%', precision: '92.08%', recall: '89.64%',
    charts: [
  ...
  {
    id: 'yolov11s-r3', round: 'ROUND 03', name: 'YOLO11s', epochs: '100', map50: '93.83%', map95: '82.06%', precision: '92.74%', recall: '88.46%',
    charts: [
...
const round4Models: ModelReport[] = [
  {
    id: 'yolov8s-r4',
    round: 'ROUND 04',
    name: 'YOLOv8s',
    ...
    recall: 'TODO',
    charts: [
      { title: 'Precision–Recall Curve', placeholder: 'TODO: 4차 YOLOv8s PR Curve 이미지 경로' },
      { title: 'F1–Confidence Curve', placeholder: 'TODO: 4차 YOLOv8s F1 Curve 이미지 경로' },
    ],
  },
  {
    id: 'yolov11s-r4',
    ...
    recall: 'TODO',
    charts: [
      { title: 'Precision–Recall Curve', placeholder: 'TODO: 4차 YOLO11s PR Curve 이미지 경로' },
      { title: 'F1–Confidence Curve', placeholder: 'TODO: 4차 YOLO11s F1 Curve 이미지 경로' },
    ],
  },
];

const analysisComparisonsByModel: Record<string, AnalysisComparison[]> = {
  'yolov11s-r3': [
    {
      example: '예시 01',
  ...
  'yolov8s-r4': [
    {
      example: '예시 01',
      label: { title: '라벨링 사진', placeholder: 'TODO: 4차 YOLOv8s 예시 01 라벨링 이미지 경로' },
      inference: { title: '추론 사진', placeholder: 'TODO: 4차 YOLOv8s 예시 01 추론 이미지 경로' },
    },
    {
      example: '예시 02',
      label: { title: '라벨링 사진', placeholder: 'TODO: 4차 YOLOv8s 예시 02 라벨링 이미지 경로' },
      inference: { title: '추론 사진', placeholder: 'TODO: 4차 YOLOv8s 예시 02 추론 이미지 경로' },
    },
  ],
  'yolov11s-r4': [
    {
      example: '예시 01',
      label: { title: '라벨링 사진', placeholder: 'TODO: 4차 YOLO11s 예시 01 라벨링 이미지 경로' },
      inference: { title: '추론 사진', placeholder: 'TODO: 4차 YOLO11s 예시 01 추론 이미지 경로' },
    },
    {
      example: '예시 02',
      label: { title: '라벨링 사진', placeholder: 'TODO: 4차 YOLO11s 예시 02 라벨링 이미지 경로' },
      inference: { title: '추론 사진', placeholder: 'TODO: 4차 YOLO11s 예시 02 추론 이미지 경로' },
    },
  ],
};

const emptyComparison: AnalysisComparison = {
  example: '예시 01',
  label: { title: '라벨링 사진', placeholder: 'TODO: 라벨링 이미지 경로' },
  inference: { title: '추론 사진', placeholder: 'TODO: 추론 이미지 경로' },
};

function ImageSlot({ image, alt }: { image: ReportImage; alt: string }) {
  if (image.src) return <img src={image.src} alt={alt} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'contain' }} />;
  return <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', padding: 24, textAlign: 'center', color: '#6f8589', fontSize: 16, fontWeight: 850 }}>{image.placeholder ?? 'TODO: 이미지 경로 추가'}</div>;
}

function ComparisonImageSlot({ image, alt }: { image: ReportImage; alt: string }) {
  if (image.src) return <img src={image.src} alt={alt} style={{ width: '100%', height: '100%', display: 'block', objectFit: 'contain' }} />;
  return <div style={{ width: '100%', height: '100%', display: 'grid', placeItems: 'center', padding: 24, boxSizing: 'border-box', textAlign: 'center', color: '#cad8db', fontSize: 17, fontWeight: 850 }}>{image.placeholder ?? 'TODO: 이미지 경로 추가'}</div>;
}

function ModelCard({ model, openReport }: { model: ModelReport; openReport: (model: ModelReport, view: ReportView) => void }) {
  return (
    <section style={{ position: 'relative', padding: '21px 23px', border: '1px solid #c8d5d9', borderRadius: 15, background: 'linear-gradient(145deg, #fff, #edf2f4)', boxShadow: '0 10px 24px rgba(29,52,62,0.07)' }}>
...
function ModelSection({ label, models, openReport }: { label: string; models: ModelReport[]; openReport: (model: ModelReport, view: ReportView) => void }) {
  return (
    <>
      <div style={{ marginTop: 18, color: '#b9502e', fontSize: 15, fontWeight: 950, letterSpacing: 1.05 }}>{label}</div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, width: '100%', marginTop: 8 }}>
        {models.map((model) => <ModelCard key={model.id} model={model} openReport={openReport} />)}
      </div>
    </>
  );
}
...
export default function Slide7() {
  const [selectedModel, setSelectedModel] = useState<ModelReport | null>(round4Models[0]);
  const [reportView, setReportView] = useState<ReportView>('charts');
  const [comparisonIndex, setComparisonIndex] = useState(0);

  const openReport = (model: ModelReport, view: ReportView) => {
    setSelectedModel(model);
    setReportView(view);
    setComparisonIndex(0);
  };

  const selectedComparisons = selectedModel
    ? analysisComparisonsByModel[selectedModel.id] ?? []
    : [];
  const currentComparison = selectedComparisons[comparisonIndex] ?? emptyComparison;
  ...
  return (
    <div className={styles.slide}>
      ...
      <div className={styles.content} style={{ justifyContent: 'flex-start', paddingTop: 94, paddingBottom: 36 }}>
        <div className={styles.chapterBadge}>AI Report · 3차 / 4차 학습</div>
        <h1 className={styles.slideTitle} style={{ marginBottom: 0 }}>AI 모델 리포트</h1>

        <ModelSection label="3차 학습" models={round3Models} openReport={openReport} />
        <ModelSection label="4차 학습" models={round4Models} openReport={openReport} />
        ...
        <div data-nav="true" onClick={(event) => { event.stopPropagation(); setSelectedModel(null); }} style={{ position: 'absolute', zIndex: 50, inset: 0, display: 'grid', placeItems: 'center', background: 'rgba(13,28,35,0.78)', backdropFilter: 'blur(7px)', cursor: 'default' }}>
          <section data-nav="true" onClick={(event) => event.stopPropagation()} style={{ width: reportView === 'charts' ? 1240 : 1320, height: reportView === 'charts' ? 680 : 740, padding: reportView === 'charts' ? '24px 26px 26px' : 0, overflow: 'hidden', boxSizing: 'border-box', borderRadius: 20, background: '#f5f7f6', boxShadow: '0 28px 70px rgba(0,0,0,0.34)' }}>
            <header style={{ minHeight: reportView === 'charts' ? 'auto' : 104, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24, marginBottom: reportView === 'charts' ? 18 : 0, padding: reportView === 'charts' ? 0 : '0 28px', borderBottom: reportView === 'charts' ? 0 : '1px solid #d4dee1', background: reportView === 'charts' ? 'transparent' : '#fff' }}>
              <div><div style={{ color: '#496b82', fontSize: 13, fontWeight: 900, letterSpacing: 1.3 }}>{selectedModel.round} · {reportView === 'charts' ? 'EVALUATION CURVES' : 'LABEL ↔ INFERENCE'}</div><h2 style={{ margin: '5px 0 0', color: '#142f3d', fontSize: 30 }}>{selectedModel.name} {reportView === 'charts' ? '평가 그래프' : '탐지 결과 비교'}</h2></div>
            ...
            {reportView === 'charts' ? (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, height: 555 }}>
                {selectedModel.charts.map((item) => <figure key={item.title} style={{ display: 'grid', gridTemplateRows: 'auto 1fr', minWidth: 0, minHeight: 0, margin: 0, padding: 15, border: '1px solid #ccd8dc', borderRadius: 13, background: '#fff' }}><figcaption style={{ marginBottom: 10, color: '#243f4b', fontSize: 18, fontWeight: 850 }}>{item.title}</figcaption><div style={{ position: 'relative', minWidth: 0, minHeight: 0 }}><ImageSlot image={item} alt={`${selectedModel.name} ${item.title}`} /></div></figure>)}
              </div>
            ) : (
